...
from model.new_pretrain import CoCLR
...
def train_one_epoch(data_loader, model, criterion, optimizer, transforms_cuda, epoch, args):
    ...
    for idx, (input_seq, vname, _) in enumerate(data_loader):
        ...
        output, output1, mask, mask1 = model(*input_seq, vname)
        mask_sum = mask.sum(1)
        mask1_sum = mask1.sum(1)
        ...
        if random.random() < 0.9:
            ...
            mask1_clone[mask1_sum != 1, 0] = 0
            loss = multi_nce_loss(output, mask_clone)
            loss1 = multi_nce_loss(output1, mask1_clone)
            loss = loss + loss1

        else:
            loss = multi_nce_loss(output, mask)
            loss1 = multi_nce_loss(output1, mask1)
            loss = loss + loss1
