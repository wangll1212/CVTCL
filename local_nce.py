...
def parse_args():
    ...
    parser.add_argument('--warmup', default=5, type=float, help='warm up')
    parser.add_argument('--warmup_array', default=[1/100,1/20,1/10,1/2,1], type=list, help='warm up')
    parser.add_argument('--scheduler_patience', default=9, type=int, help='warm up')
    parser.add_argument('--schedule', default=[250, 280], nargs='*', type=int, help='learning rate schedule (when to drop lr by 10x)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        random.seed(epoch)
        if epoch < args.warmup:   # epoch<5
            lr = args.warmup_array[epoch]*args.lr   # lr=[1/100,1/20,1/10,1/2,1]*lr
...
def train_one_epoch(data_loader, model, criterion, optimizer, transforms_cuda, epoch, args):
    ...
    def tr(x,num_seq,seq_len):
        B = x.size(0)
        return transforms_cuda(x).view(B,3,num_seq,seq_len,args.img_dim,args.img_dim)\
        .transpose(1,2).contiguous()

    tic = time.time()
    end = time.time()

    for idx, (input_seq, clip1, clip2, clip3, clip4, label) in tqdm(enumerate(data_loader), total=len(data_loader), disable=True):
        data_time.update(time.time() - end)
        B = input_seq.size(0)
        # print(input_seq.size())  # torch.Size([16, 3, 64, 128, 128])
        # print(clip1.size())   # torch.Size([16, 3, 16, 128, 128])
        # assert clip1.size(3) == 16
        input_seq = tr(input_seq.cuda(non_blocking=True),args.num_seq,args.seq_len)  # (B,2,3,32,128,128)
        clip1 = tr(clip1.cuda(non_blocking=True),args.num_seq,8)    # (B,2,3,8,128,128)
        clip2 = tr(clip2.cuda(non_blocking=True),args.num_seq,8)
        clip3 = tr(clip3.cuda(non_blocking=True),args.num_seq,8)
        clip4 = tr(clip4.cuda(non_blocking=True),args.num_seq,8)

        # 损失计算
        # 'target' is the index of self
        output, output_local, output_intra, target, target_local, target_intra = model(input_seq, clip1, clip2, clip3, clip4)
        loss_global = criterion(output, target)
        loss_local = criterion(output_local, target_local)/3
        loss_intra = criterion(output_intra, target_intra)
        loss = loss_global + loss_local + loss_intra
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    for milestone in args.schedule:    # [250 280]
        lr *= 0.1 if epoch >= milestone else 1.   # 学习率减小10倍，0.00001
