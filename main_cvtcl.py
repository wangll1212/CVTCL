...
import utils.augmentation_local as A
...
from model.local_pretrain import CoCLR
...
def train_one_epoch(data_loader, model, criterion, optimizer, transforms_cuda, epoch, args):
    ...
    def tr(x,num_seq,seq_len):
        B = x.size(0)
        # [8, 3, 2, 32, 128, 128]
        return transforms_cuda(x).view(B,3,num_seq,seq_len,args.img_dim,args.img_dim).transpose(1,2).contiguous()  
    tic = time.time()
    end = time.time()

    
    for idx, (input_seq, clip1, clip2, clip3, clip4, vname, _) in enumerate(data_loader):
        data_time.update(time.time() - end)
        B = input_seq[0].size(0)
        # print(input_seq[0].size())  # torch.Size([B, 3, 64, 128, 128])
        # print(clip1[0].size())   # torch.Size([B, 3, 16, 128, 128])

        input_seq = [tr(i.cuda(non_blocking=True),args.num_seq,args.seq_len) for i in input_seq] # (B,3,2,32,128,128) 将1,2维交换
        clip1 = [tr(i.cuda(non_blocking=True),args.num_seq,8) for i in clip1]   # (B,3,2,8,128,128)
        clip2 = [tr(i.cuda(non_blocking=True),args.num_seq,8) for i in clip2]
        clip3 = [tr(i.cuda(non_blocking=True),args.num_seq,8) for i in clip3]
        clip4 = [tr(i.cuda(non_blocking=True),args.num_seq,8) for i in clip4]
        # input = (input_seq[0], input_seq[1], clip1[0], clip1[1], clip2[0], clip2[1], clip3[0],clip3[1], clip4[0], clip4[1])
        vname = vname.cuda(non_blocking=True)

        # output, output1, output2, mask, labels1, labels2 = model(*input_seq, clip1[0], clip1[1], clip2[1], clip3[1], clip4[1], vname)  
        # *parameter是用来接受任意多个参数并将其放在一个元组中
        output, output1, output2, mask, labels1, labels2 = model(*input_seq, *clip1, *clip2, *clip3, *clip4, vname)
        mask_sum = mask.sum(1)
