...
class TwoClipTransform:
    ...
    def __call__(self, x):
        ...
        # print("twoclip片段的长度为：%s" % len(x))
        if len(x)==128:
            assert len(x) == 2 * self.seq_len
        elif len(x) == 32:
            assert len(x) ==  2 * self.seq_len/4
        else:
            print("片段长度为：%s" % len(x))
            assert 1 == 2
...
class OneClipTransform:
    ...
    def __call__(self, x ):
        # target: list of image
        # print("oneclip片段的长度为：%s" % len(x))
        # print("seq_len为：%s" % self.seq_len) # 64
        if len(x)==128:
            assert len(x) == 2 * self.seq_len    # 2*64=128
        elif len(x) == 32:
            assert len(x) == 2 * self.seq_len/4  # 2*16=32
        else:
            # print("oneclip片段的长度为：%s" % len(x))
            assert 1 == 2
